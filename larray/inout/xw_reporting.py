...
class AbstractReportSheet(AbstractReportItem):
    ...
    def add_graph(self, data, title=None, template=None, width=None, height=None, **kwargs):
        r"""
        ...
        **kwargs
          ...
          * func: function, optional
              ...
              All remaining non-used keyword arguments are passed to the function at call.
...
if xw is not None:
    ...
    class ExcelGraphItem(ItemSize):
        ...
        def __init__(self, data, title, template, top, left, width, height, **kwargs):
            ItemSize.__init__(self, width, height)
            self.top = top
            self.left = left
        ...
        def dump(self, sheet, data_sheet, row):
            ...
            # periodicity
            periodicity = kwargs.pop('periodicity', None)
            if periodicity is not None:
                obj_chart.Axes(AxisType.xlCategory).TickLabelSpacing = periodicity
                obj_chart.Axes(AxisType.xlCategory).TickMarkSpacing = periodicity
                obj_chart.Axes(AxisType.xlCategory).TickLabelPosition = Constants.xlLow
            # user's function (to apply on remaining kwargs)
            func = kwargs.pop('func', None)
            if func is not None:
                func(obj_chart, **kwargs)
            # flagflip
    ...
    class ReportSheet(AbstractReportSheet):
        ...
        def add_graph(self, data, title=None, template=None, width=None, height=None, **kwargs):
            ...
            if self.graphs_per_row is not None and self.position_in_row > self.graphs_per_row:
                self.newline()
            self.items.append(ExcelGraphItem(data, title, template, self.top, self.left, width, height, **kwargs))
            self.left += width
        ...
        def add_graphs(self, array_per_title, axis_per_loop_variable, template=None, width=None, height=None,
            ...
            for loop_variable_values, arrays_chunk in zip_array_items(arrays, axes=axes):
                ...
                for title_template, array_chunk in zip(titles, arrays_chunk):
                    ...
                    self.add_graph(array_chunk, title, template, width, height, **kwargs)
            if graphs_per_row is not None:
